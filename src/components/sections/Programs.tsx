...
export default function Programs() {
  ...
  const additionalPrograms = programs.filter((p) => !["bjj-gi", "no-gi", "mma"].includes(p.id));

  return (
    <>
    {/* Programs cards — GI / No-GI / MMA are rendered above as scroll sections in page.tsx */}
    {additionalPrograms.length > 0 ? (
      <section id="programs-cards" className="flex flex-col items-center justify-start pb-0 md:pb-24 bg-background-elevated relative overflow-hidden">
    ...
    >
      ...
      >
        ...
        <motion.div
          ...
          className="mt-0 md:relative md:left-1/2 md:w-screen md:-translate-x-1/2 md:mt-24"
        >
          <div className="w-full border-gradient-gold overflow-hidden flex flex-col md:flex-row">
            <h3
              className="block md:hidden px-6 pt-8 pb-4 text-center text-white leading-[0.92] text-[clamp(2.2rem,10.5vw,3.2rem)]"
            ...
            {/* Image */}
            <motion.div
              className="relative h-[22rem] md:h-auto md:w-1/2 md:min-h-[760px]"
        ...
        <motion.div
          ...
          className="mt-0 md:relative md:left-1/2 md:w-screen md:-translate-x-1/2"
        >
          <div className="w-full border-gradient-gold overflow-hidden flex flex-col md:flex-row">
            <h3
              className="block md:hidden px-6 pt-4 pb-2 text-center text-white leading-[0.92] text-[clamp(2.2rem,10.5vw,3.2rem)]"
              style={{ fontFamily: "var(--font-display)" }}
            >
              VETERAN & LEO PROGRAMS
            </h3>
            {/* Image — shown second on mobile (after title), right side on desktop */}
            <motion.div
              className="relative h-[22rem] md:h-auto md:w-1/2 md:min-h-[760px] order-2 md:order-none"
              initial={{ opacity: 0, x: 24, scale: 1.04 }}
              whileInView={{ opacity: 1, x: 0, scale: 1 }}
              viewport={{ once: true, amount: 0.45 }}
              transition={{ duration: 0.75, ease: [0.22, 1, 0.36, 1] }}
